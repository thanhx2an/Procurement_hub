...
async function run() {
    // Load compiled model so CDS knows about all services and external APIs
    const csn = await cds.load('srv/csn.json');
    cds.model = await cds.linked(csn);

    const db = await cds.connect.to('db');
    const S4_PROD = await cds.connect.to('API_PRODUCT');

    // 1. Find all entries with basePrice = 0 or null (use string entity name — no model load needed)
    const entries = await db.run(
        SELECT.from('hub.procurement.PriceLedger')
    ...
    for (const entry of entries) {
        ...
        await db.run(
            UPDATE('hub.procurement.PriceLedger')
                .set({ basePrice: price })
                .where({ ID: entry.ID })
        );
        updated++;
